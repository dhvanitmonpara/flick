...
function ShareButton({ id }: { id: string }) {
  ...
  return (
    <button
      ...
      aria-label="Share"
    >
      {shared ? <FaCheck className="text-green-500 text-base" /> : <PiShareFatFill className="text-zinc-500 dark:text-zinc-400 group-hover:text-zinc-700 dark:group-hover:text-zinc-300 group-hover:scale-110 transition-all duration-300 text-base" />}
    </button>
  )
}
