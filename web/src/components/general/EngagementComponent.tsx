...
const EngagementComponent = ({
  ...
  const handleVote = async (type: "upvote" | "downvote") => {
    ...
    setOptimisticCounts((prev) =>
      getUpdatedCounts(prev, upvoted, downvoted, type),
    );
    ...
    <div className="flex flex-wrap items-center gap-3">
      <div className="flex items-center bg-zinc-200 dark:bg-zinc-800 rounded-full p-0.5 shadow-sm">
        {show.includes("upvotes") && (
          ...
          >
            <PiArrowFatUpFill
              className={`pointer-events-none text-lg transition-transform ${upvoted ? "text-blue-500" : "text-zinc-500 dark:text-zinc-400"} group-hover:scale-110 group-hover:text-blue-500`}
            />
            <span
              className={`pointer-events-none text-sm font-medium pr-1 select-none transition-colors ${upvoted ? "text-blue-500" : "text-zinc-600 dark:text-zinc-400 group-hover:text-blue-500"}`}
            >
              {optimisticCounts.upvotes ?? 0}
            </span>
          </button>
        )}
        {show.includes("upvotes") && show.includes("downvotes") && (
          <Separator
            ...
            className="h-4! my-auto w-[1.5px] bg-zinc-300 dark:bg-zinc-700 mx-1"
          />
        )}
        {show.includes("downvotes") && (
          ...
          >
            <PiArrowFatDownFill
              className={`pointer-events-none text-lg transition-transform ${downvoted ? "text-red-500" : "text-zinc-500 dark:text-zinc-400"} group-hover:scale-110 group-hover:text-red-500`}
            />
            <span
              className={`pointer-events-none text-sm font-medium pr-1 select-none transition-colors ${downvoted ? "text-red-500" : "text-zinc-600 dark:text-zinc-400 group-hover:text-red-500"}`}
            >
              {optimisticCounts.downvotes ?? 0}
            </span>
          </button>
        )}
      </div>
      ...
      {show.includes("comments") && isLoggedIn && (
        <CommentButton parentCommentId={targetType === "comment" ? id : null}>
          ...
          >
            ...
            <span className="pointer-events-none text-sm font-medium text-zinc-600 dark:text-zinc-400 pr-0.5 select-none">
              {optimisticCounts.comments ?? 0}
            </span>
          </button>
        </CommentButton>
      )}
      ...
      {show.includes("views") && (
        <button
          onClick={(e) => {
            e.stopPropagation();
            toast.info("Analytics feature is under development");
          }}
        ...
        >
          ...
          <span className="pointer-events-none text-sm font-medium text-zinc-600 dark:text-zinc-400 pr-0.5 select-none">
            {optimisticCounts.views ?? 0}
          </span>
        </button>
      )}

      {show.includes("share") && <ShareButton id={id} />}
    </div>
  );
};
