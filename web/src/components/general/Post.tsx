...
function Post({
  ...
  return (
    <Card
      onClick={(e) => handleLinkClick(e, `/p/${id}`)}
      className="dark:bg-transparent bg-transparent border-none shadow-none rounded-none"
    >
      <CardHeader className="flex flex-row justify-between space-x-2 px-4 pt-4">
        <div className="flex space-x-4">
          <VisuallyHidden>
            <CardTitle onClick={(e) => handleLinkClick(e, `/p/${id}`)}>
              {title}
            </CardTitle>
            <CardDescription onClick={(e) => handleLinkClick(e, `/p/${id}`)}>
              {content}
            </CardDescription>
          </VisuallyHidden>
          <Avatar>
          ...
          <div className="space-y-1">
            <h2 className="flex flex-wrap items-center gap-x-0.5 gap-y-1 font-semibold text-zinc-900 dark:text-zinc-100">
              <Link
                onClick={(e) => handleLinkClick(e, `/?branch=${branch}`)}
            ...
            <p className="flex flex-wrap items-center gap-x-0.5 gap-y-1 text-xs text-zinc-600 dark:text-zinc-400">
              <Link
                onClick={(e) => handleLinkClick(e, `/college/${college}`)}
                className="hover:underline"
                href={`/college/${college}`}
              >
                {college}
              </Link>
              <BsDot size={16} />
              <Link
                onClick={(e) => handleLinkClick(e, `/user/${username}`)}
                className="hover:underline"
                href={`/user/${username}`}
              >
                {username}
              ...
              <Link
                onClick={(e) =>
                  handleLinkClick(e, `/?topic=${parseTopic(topic)}`)
                }
                className="hover:underline"
                href={`/?topic=${parseTopic(topic)}`}
              >
                {topic}
              </Link>
              {isPrivate && (
      ...
      <CardFooter>
        <EngagementComponent
          ...
          key={title}
          show={["upvotes", "downvotes", "comments", "views", "share"]}
        />
      </CardFooter>
    </Card>
  );
