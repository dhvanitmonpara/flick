...
function MobileNav() {
  ...
  return (
    <div className="md:hidden fixed bottom-0 left-0 right-0 border-t border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-950 flex justify-around items-center h-14 z-50">
      <Link
        href="/"
        className={`flex flex-col items-center justify-center w-full h-full ${pathname === "/" || pathname.startsWith("/?topic=") || pathname.startsWith("/?branch=") ? "text-zinc-900 dark:text-zinc-100" : "text-zinc-500 hover:text-zinc-900 dark:hover:text-zinc-100"}`}
      >
        {pathname === "/" ||
        pathname.startsWith("/?topic=") ||
        pathname.startsWith("/?branch=") ? (
          <PiCardsThreeFill size={26} />
        ) : (
          <PiCardsThreeLight size={26} />
        )}
      </Link>
      <Link
        href="/college"
        className={`flex flex-col items-center justify-center w-full h-full ${pathname === "/college" ? "text-zinc-900 dark:text-zinc-100" : "text-zinc-500 hover:text-zinc-900 dark:hover:text-zinc-100"}`}
      >
        {pathname === "/college" ? (
      ...
      <div className="flex items-center justify-center w-full h-full">
        {isLoggedIn ? (
          <CreatePost>
            <button
              type="button"
              className="flex items-center justify-center bg-zinc-900 dark:bg-zinc-100 text-white dark:text-zinc-900 rounded-full w-10 h-10"
            >
              <FaPlus size={16} />
            </button>
          </CreatePost>
      ...
      <Link
        href="/trending"
        className={`flex flex-col items-center justify-center w-full h-full ${pathname === "/trending" ? "text-zinc-900 dark:text-zinc-100" : "text-zinc-500 hover:text-zinc-900 dark:hover:text-zinc-100"}`}
      >
        {pathname === "/trending" ? (
          <PiFireFill size={26} />
        ) : (
      ...
      >
        <img
          ...
          src={
            isLoggedIn
              ? (useProfileStore.getState().profile.college as any)?.profile ||
                `https://api.dicebear.com/9.x/initials/svg?seed=${useProfileStore.getState().profile.username || "U"}`
              : theme === "dark"
                ? "/logo-w.png"
                : "/logo-b.png"
          }
      ...
      <div className="hidden md:block min-w-[270px] w-[270px] h-screen py-6 overflow-y-auto no-scrollbar space-y-1 px-4">
        ...
        <section className="space-y-2">
          <Tab
            to="/"
            text="Feed"
            activeIcon={<PiCardsThreeFill size={22} />}
            passiveIcon={<PiCardsThreeLight size={22} />}
          />
          <Tab
            to="/college"
            text="My College"
            activeIcon={<RiGraduationCapFill size={22} />}
            passiveIcon={<RiGraduationCapLine size={22} />}
          />
          <Tab
            to="/trending"
            text="Trending"
            activeIcon={<PiFireFill size={22} />}
            passiveIcon={<PiFireLight size={22} />}
          />
        </section>
        ...
        {showAllBranches ? (
          <>
            {PostBranches.map((branch) => (
              <Tab
                key={branch}
                to={`/?branch=${branch.toLowerCase().replace(" ", "+")}`}
                text={branch}
              />
            ))}
            <button
              type="button"
              className="flex justify-center items-center space-x-1 cursor-pointer text-gray-600 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-300 pl-4 py-2"
...
function Heading({ text }: { text: string }) {
  return <h3 className="text-lg uppercase px-4 pt-3 pb-1">{text}</h3>;
}
