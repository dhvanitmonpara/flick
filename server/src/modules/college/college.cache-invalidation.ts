...
export async function invalidateCollegeCaches(
	options: InvalidateCollegeCacheOptions = {},
) {
	const ops: Array<Promise<unknown>> = [
		cache.incr(collegeCacheKeys.listVersionKey()),
	];
	...
	if (options.previousEmailDomain) {
		ops.push(
			cache.del(collegeCacheKeys.emailDomain(options.previousEmailDomain)),
		);
	}
