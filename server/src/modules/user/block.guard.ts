...
export const assertNoBlockRelationBetweenUsers = async (
  ...
  const hasBlockRelation = await UserRepo.Blocks.hasBlockRelation(requester.authId, target.authId, dbTx);

  if (hasBlockRelation) {
    throw HttpError.forbidden("You cannot interact with this user due to block settings", {
