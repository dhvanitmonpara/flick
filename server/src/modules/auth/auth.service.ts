...
class AuthService {
	...
	initializeRegistration = async (email: string, res: Response) => {
	...
	finishRegistration = async (
		...
		if (existingAuth) {
			...
			await db
				...
				.where(eq(authTable.id, createdUser.id));
	...
	cleanupOrphanedAuthUser = async (authId: string, res?: Response) => {
		logger.warn(
			"Cleaning up orphaned auth user after failed profile provisioning",
			{
				authId,
	...
	deleteAccount = async (
		req: Request,
		res: Response,
		_payload: { password?: string; token?: string; callbackURL?: string },
	) => {
		const headers = parseHeaders(req.headers);
		...
		if (profile) {
			await db
				...
				.where(eq(notifications.receiverId, profile.id));
		...
		if (authResponse) {
			forwardSetCookieHeaders(
				(authResponse as unknown as globalThis.Response).headers,
				res,
			);
		} else {
	...
	getAllUsersForAdmin = async (options: {
		query?: string;
		limit?: number;
		offset?: number;
	}) => {
	...
	hasPassword = async (authId: string): Promise<boolean> => {
		const [row] = await db
			...
			.from(account)
			.where(
				and(eq(account.userId, authId), eq(account.providerId, "credential")),
			)
			.limit(1);
		return !!row?.password;
	};
