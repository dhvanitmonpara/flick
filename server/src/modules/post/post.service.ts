...
class PostService {
	...
	async createPost(postData: {
		...
		const postWithDetails = await PostRepo.Read.findByIdWithDetails(
			newPost.id,
			postData.postedBy,
		);
		...
		if (post.isPrivate) {
			if (!user) {
				throw HttpError.unauthorized(
					"Please log in to view this college-only post.",
					{
			...
			if (user.collegeId !== post.postedBy?.college?.id) {
				throw HttpError.forbidden(
					"You do not have access to view this college-only post.",
					{
	...
	async updatePost(
		...
		if (existingPost.postedBy !== userId) {
			logger.warn("Unauthorized post update attempt", {
				postId: id,
				userId,
				authorId: existingPost.postedBy,
			});
	...
	async incrementPostViews(id: string) {
		...
		const _ctx = observabilityContext.getStore();

		const updatedPost = await PostRepo.Write.incrementViews(id);
		return updatedPost;
	}

	async getPostsByCollege(
		collegeId: string,
		options?: {
			page?: number;
			limit?: number;
