...
const PostRepo = {
	CachedRead: {
		...
		findByIdWithDetails: async (id: string, userId?: string, dbTx?: DB) =>
			cached(await postCacheKeys.idWithDetails(id, userId), () =>
				PostAdapter.findByIdWithDetails(id, userId, dbTx),
			),
		...
		countAll: async (
			...
			cached(
				await postCacheKeys.count(
					filters?.topic,
					filters?.collegeId,
					filters?.branch,
					filters?.userCollegeId,
					filters?.authorId,
					filters?.blockerAuthId,
				),
