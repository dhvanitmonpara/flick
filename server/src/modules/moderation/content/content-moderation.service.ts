...
import { CommentAdapter, PostAdapter } from "@/infra/db/adapters";
import ContentReportService from "../reports/report-moderation.service";

// biome-ignore lint/complexity/noStaticOnlyClass: <this is the part of design pattern>
class ContentModerationService {
	static async banPost(postId: string) {
		...
		const updatedPost = await PostAdapter.updateById(postId, {
			isBanned: true,
		});
		if (!updatedPost) {
		...
		await ContentReportService.updateReportsByTargetId(
			postId,
			"Post",
			"resolved",
		);

		logger.info("Post banned successfully", {
			postId,
	...
	static async banComment(commentId: string) {
		...
		const updatedComment = await CommentAdapter.updateById(commentId, {
			isBanned: true,
		});
		if (!updatedComment) {
	...
	static async moderateContent(
		...
		switch (type) {
			case "Post":
				switch (action) {
					case "ban":
						return await ContentModerationService.banPost(targetId);
					case "unban":
						return await ContentModerationService.unbanPost(targetId);
					case "shadowBan":
						return await ContentModerationService.shadowBanPost(targetId);
					case "shadowUnban":
						return await ContentModerationService.shadowUnbanPost(targetId);
					default:
			...
			case "Comment":
				switch (action) {
					case "ban":
						return await ContentModerationService.banComment(targetId);
					case "unban":
						return await ContentModerationService.unbanComment(targetId);
					case "shadowBan":
						throw HttpError.badRequest("Shadow ban not supported for comments");
					case "shadowUnban":
						throw HttpError.badRequest(
							"Shadow unban not supported for comments",
						);
					default:
