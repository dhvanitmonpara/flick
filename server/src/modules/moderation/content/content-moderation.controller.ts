...
class ModerationController {
  static async upsertPostState(req: Request) {
    ...
    const { state } = moderationSchemas.postModerationStateSchema.parse(req.body);

    if (state === "active") {
      await tryBestEffort(() => ContentModerationService.unbanPost(postId));
      await tryBestEffort(() => ContentModerationService.shadowUnbanPost(postId));
    }
  ...
  static async upsertCommentState(req: Request) {
    const { commentId } = moderationSchemas.commentIdParamsSchema.parse(req.params);
    const { state } = moderationSchemas.commentModerationStateSchema.parse(req.body);

    if (state === "active") {
      await tryBestEffort(() => ContentModerationService.unbanComment(commentId));
    }
