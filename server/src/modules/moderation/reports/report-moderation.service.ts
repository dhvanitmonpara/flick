...
import type { ContentReportInsert } from "@/infra/db/tables/content-report.table";
...
class ContentReportService {
	static async createReport(values: ContentReportInsert) {
		if (shouldSampleLog(values.reportedBy))
			logger.info("Creating content report", {
				type: values.type,
	...
	static async getReportsWithFilters(filters: {
		...
		const { reports, totalCount } =
			await ContentReportRepo.Read.findWithFilters(filters);
	...
	static async updateReportsByTargetId(
		...
		const report = await ContentReportRepo.Write.updateManyByTargetId(
			targetId,
			type,
			status,
		);
