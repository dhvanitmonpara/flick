...
const CommentRepo = {
	CachedRead: {
		...
		findAuthorId: (id: string, dbTx?: DB) =>
			cached(commentCacheKeys.authorId(id), () =>
				CommentAdapter.findAuthorId(id, dbTx),
			),

		findByIdWithAuthor: (id: string, dbTx?: DB) =>
			cached(commentCacheKeys.id(id), () =>
				CommentAdapter.findByIdWithAuthor(id, dbTx),
			),
		...
		findByPostId: async (
			...
			return cached(
				await commentCacheKeys.postComments(
					postId,
					page,
					limit,
					sortBy,
		...
		countByPostId: async (postId: string, dbTx?: DB) =>
			cached(await commentCacheKeys.postCount(postId), () =>
				CommentAdapter.countByPostId(postId, dbTx),
			),
	},
