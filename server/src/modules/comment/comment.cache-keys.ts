...
const commentCacheKeys = {
	...
	postCount: async (postId: string) => {
		const version =
			(await cache.get<number>(`post:${postId}:comments:version`, {
				bypassL1: true,
			})) || 1;
		return `comment:count:post:${postId}:v${version}`;
	},
	postCommentsVersionKey: (postId: string) => `post:${postId}:comments:version`,
	commentRepliesVersionKey: (commentId: string) =>
		`comment:${commentId}:replies:version`,
	postComments: async (
		postId: string,
		page: number,
		limit: number,
		...
		const version =
			(await cache.get<number>(versionKey, { bypassL1: true })) || 1;
	...
	commentReplies: async (commentId: string, limit: number = 10) => {
