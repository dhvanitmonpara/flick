...
class CommentController {
	static async createComment(req: Request) {
		const { content, parentCommentId } =
			commentSchemas.CreateCommentSchema.parse(req.body);
		...
		return HttpResponse.created("Comment created successfully", {
			comment: newComment,
		});
	}
	...
	static async updateComment(req: Request) {
		...
		return HttpResponse.ok("Comment updated successfully", {
	...
	static async getCommentsByPostId(req: Request) {
		...
		const { page, limit, sortBy, sortOrder } =
			commentSchemas.GetCommentsQuerySchema.parse(req.query);
