...
const AdminRepo = {
	Read: {
		getManageUsersQuery: (username?: string, email?: string, dbTx?: DB) =>
			AdminAdapter.getManageUsersQuery(username, email, dbTx),
		...
		getLogs: (
			...
			sortOrder: "asc" | "desc",
			dbTx?: DB,
		) => AdminAdapter.getLogs(page, limit, sortBy, sortOrder, dbTx),
	...
	Write: {
		createCollege: (
			...
			dbTx?: DB,
		) => AdminAdapter.createCollege(data, dbTx),
		updateCollege: (
			id: string,
			updates: Partial<{
				name: string;
