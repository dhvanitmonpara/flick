...
export const UpdateFeedbackStatusSchema = z.object({
	status: z.enum(["new", "reviewed", "dismissed"], {
		error: () => ({
			message: "Status must be one of: new, reviewed, dismissed",
		}),
	}),
});
...
export const ListFeedbacksQuerySchema = z.object({
	limit: z
		.string()
		.transform((val) => Math.max(1, Math.min(100, parseInt(val, 10) || 50)))
		.optional(),
	skip: z
		.string()
		.transform((val) => Math.max(0, parseInt(val, 10) || 0))
		.optional(),
