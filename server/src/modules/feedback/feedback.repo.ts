...
const FeedbackRepo = {
	CachedRead: {
		findById: (id: string, dbTx?: DB) =>
			cached(feedbackCacheKeys.id(id), () =>
				FeedbackAdapter.findById(id, dbTx),
			),

		findByIdWithUser: (id: string, dbTx?: DB) =>
			cached(feedbackCacheKeys.id(id), () =>
				FeedbackAdapter.findByIdWithUser(id, dbTx),
			),
		...
		countAll: async (
			...
			cached(
				await feedbackCacheKeys.count(filters?.type, filters?.status),
