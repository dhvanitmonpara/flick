...
const BookmarkRepo = {
	Read: {
		findBookmark: (userId: string, postId: string, dbTx?: DB) =>
			BookmarkAdapter.findByUserAndPostId(userId, postId, dbTx),

		findBookmarkWithPost: (userId: string, postId: string, dbTx?: DB) =>
			BookmarkAdapter.findByUserAndPostIdWithPost(userId, postId, dbTx),

		getUserBookmarkedPosts: (userId: string, dbTx?: DB) =>
			BookmarkAdapter.findBookmarkedPostsByUserId(userId, dbTx),
	},

	CachedRead: {
		findBookmark: (userId: string, postId: string, dbTx?: DB) =>
			cached(bookmarkCacheKeys.multiId(userId, postId), () =>
				BookmarkAdapter.findByUserAndPostId(userId, postId, dbTx),
			),
		...
		getUserBookmarkedPosts: (userId: string, dbTx?: DB) =>
			cached(bookmarkCacheKeys.id(userId), () =>
				BookmarkAdapter.findBookmarkedPostsByUserId(userId, dbTx),
			),
	},
