...
class BookmarkService {
	async createBookmark(userId: string, postId: string) {
		...
		const newBookmark = await BookmarkRepo.Write.createBookmark({
			userId,
			postId,
		});
	...
	async getUserBookmarkedPosts(userId: string) {
		...
		const bookmarks =
			await BookmarkRepo.CachedRead.getUserBookmarkedPosts(userId);
		...
		const blockedUserIds =
			await UserRepo.Blocks.getBlockedUserIdsInEitherDirection(user.authId);
	...
	async deleteBookmark(userId: string, postId: string) {
		...
		if (!deleted) {
			...
			throw HttpError.notFound("Bookmark not found for this user", {
				...
				errors: [
					{ field: "postId", message: "Bookmark not found for this user" },
				],
			});
		}
	...
	async getBookmark(userId: string, postId: string) {
		...
		const bookmark = await BookmarkRepo.CachedRead.findBookmarkWithPost(
			userId,
			postId,
		);
		if (!bookmark) {
		...
		logger.info("Bookmark retrieved successfully", {
			userId,
			postId,
