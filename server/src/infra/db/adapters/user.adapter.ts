...
export const create = async (data: typeof users.$inferInsert, dbTx?: DB) => {
	const client = dbTx ?? db;
...
export const findByQuery = async (
	...
	if (filters.username) {
		conditions.push(
			ilike(users.username, `%${filters.username.trim().toLowerCase()}%`),
		);
	}
...
export const hasBlockRelation = async (
	firstAuthId: string,
	secondAuthId: string,
	dbTx?: DB,
) => {
...
export const getBlockedUserIdsInEitherDirection = async (
	authId: string,
	dbTx?: DB,
) => {
	const client = dbTx ?? db;
