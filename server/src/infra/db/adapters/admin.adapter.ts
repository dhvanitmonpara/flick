/** biome-ignore-all lint/suspicious/noExplicitAny: <data is not of type any> */
...
export const getManageUsersQuery = async (
	username?: string,
	email?: string,
	dbTx?: DB,
) => {
	...
	const rawReports = await client
		...
		.where(
			statuses && statuses.length > 0
				? inArray(contentReports.status, statuses as any)
				: undefined,
		)
...
export const getLogs = async (
	...
	if (sortBy === "action") orderByColumn = auditLogs.action;
		...
		orderByColumn = auditLogs.occuredAt;
...
export const updateCollege = async (
	...
	const { branches: _branches, ...collegeUpdates } = updates as Partial<
