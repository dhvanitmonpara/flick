...
import { colleges, comments, users, votes } from "../tables";
...
export const findAuthorId = async (id: string, dbTx?: DB) => {
	...
	const result = await client
		.select({ postedBy: comments.postedBy })
		.from(comments)
		.where(eq(comments.id, id))
		.limit(1);
...
export const findByPostId = async (
	...
	const orderBy =
		sortOrder === "asc" ? asc(comments[sortBy]) : desc(comments[sortBy]);
...
export const countByPostId = async (postId: string, dbTx?: DB) => {
	...
	const result = await client
		...
		.from(comments)
		.where(and(eq(comments.postId, postId), eq(comments.isBanned, false)));

	return result[0]?.count || 0;
};

export const create = async (
	comment: typeof comments.$inferInsert,
	dbTx?: DB,
) => {
	const client = dbTx ?? db;
