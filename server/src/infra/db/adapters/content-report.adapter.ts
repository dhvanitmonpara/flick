...
import {
	...
	type ContentReportSelect,
	contentReports,
} from "../tables/content-report.table";
...
export const findById = async (
	id: string,
	db?: DBTx,
): Promise<ContentReportSelect | null> => {
	const client = db ?? prisma;
	const [report] = await client
		.select()
		.from(contentReports)
...
export const findByTargetId = async (
	...
	const whereCondition =
		type === "Post"
			? and(
					eq(contentReports.postId, targetId),
					eq(contentReports.type, "Post"),
				)
			: and(
					eq(contentReports.commentId, targetId),
					eq(contentReports.type, "Comment"),
				);
