...
import { auth, platformUser, verification } from "@/infra/db/tables/auth.table";
...
export const create = async (data: typeof auth.$inferInsert, dbTx?: DB) => {
	const client = dbTx ?? db;
...
export const deleteById = async (authId: string, dbTx?: DB) => {
	try {
		const client = dbTx ?? db;
...
export const searchUsers = async (options: SearchUsersOptions, dbTx?: DB) => {
	const client = dbTx ?? db;

	const { query, limit = 10, offset = 0, collegeId } = options;
...
export const listUsersForAdmin = async (
	...
	const client = dbTx ?? db;
	const { query, limit = 20, offset = 0, roles } = options;
