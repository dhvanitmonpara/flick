...
function Header() {
  ...
  return (
    <div
      className={`sticky flex justify-between items-center transition-all duration-300 mx-auto z-50 ${
        scrolled
    ...
    >
      ...
      <div
        className={`flex justify-center items-center ${scrolled ? "gap-8" : "gap-12"} animate-fade-in-blur`}
      >
        <Link
          className="hover:text-primary hover:font-semibold hidden sm:block"
          href="/"
        >
          Features
        </Link>
        ...
        {scrolled && (
          <CTAButton
            size="lg"
            className="animate-slide-in-left hidden sm:flex"
          />
        )}
      </div>
    </div>
