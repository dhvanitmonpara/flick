...
- Enumerations: Centralized custom ENUM types for audit, notifications, topics, votes, and content reports.
...
- Enums: [enums.ts](file://server/src/infra/db/tables/enums.ts#L1-L49)
...
### Colleges
Colleges are referenced by users for affiliation.
...
COLLEGES ||--o{ PLATFORM_USER : "enrolls"
```
...
## Dependency Analysis
The schema enforces referential integrity via foreign keys and Drizzle relations. The following diagram highlights key dependencies among entities.
...
- Unique constraints on identifiers (email, username).
...
- Unique indexes to prevent duplicates (e.g., votes).
