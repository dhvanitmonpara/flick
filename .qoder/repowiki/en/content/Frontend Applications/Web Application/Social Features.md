...
## Core Components
- CreatePost: A modal-based form for creating or editing posts with Zod validation, term acceptance flow, and integration with post API and Zustand store.
- Post: A lightweight card component rendering post metadata, content preview, and engagement controls.
...
- Backend: REST endpoints expose CRUD operations for posts, comments, votes, and bookmarks. Content moderation/reporting integrates with reporting workflows.
...
- Validation: Zod schema enforces title length, content length, topic selection, and optional privacy flag.
- Workflow:
...
OpenDialog --> InitForm["Init Form (Zod Resolver)"]
InitForm --> Submit["User submits form"]
Submit --> Validate["Validate schema"]
...
- Interaction: Clicking the card navigates to the post detail route; metadata links navigate to filters.
